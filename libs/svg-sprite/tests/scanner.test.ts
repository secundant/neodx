...
describe('scanner', () => {
  test('should scan 2 files', async () => {
    const scanner = createScanner('fixtures/simple');
  ...
  test('should return actual files content', async () => {
    ...
    expect(files.map(file => file.content)).toEqual([
      await readFile(resolveRoot('fixtures/simple/ArrowDropDown.svg'), 'utf-8'),
      await readFile(resolveRoot('fixtures/simple/ArrowDropUp.svg'), 'utf-8')
    ]);
  });
