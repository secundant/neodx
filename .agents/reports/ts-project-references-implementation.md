...
| **S5-R2-b** | Retry Oxlint `typeAware`/`typeCheck` (#161)     | **Unblocked + probed.** R2-a deleted base `baseUrl`/`paths`; a follow-up commit (`259d543`) dropped the vestigial `baseUrl`/`paths:{}` from all 12 `tsconfig.build.json`. tsgolint no longer hard-rejects on the **lib** tree. Probe (`typeAware:true`) cleared lib `baseUrl` errors; residual blockers for the full retry: (1) `baseUrl` still in `apps/examples/*/tsconfig.json` + `libs/autobuild/examples/*/tsconfig.json`; (2) ~72 `typescript(...)` violations (`unbound-method`, `no-useless-default-assignment`, `no-redundant-type-constituents` incl. `Buffer` "error type" — likely a missing `@types/node`/lib config under tsgolint). Triage these in a dedicated session. | typeAware on in CI, or residual issue filed |
