...
## Program status (2026-08-07)

Critical path on `improve/neodx` (PR [#160](https://github.com/secundant/neodx/pull/160)).
Re-read `git rev-parse --short HEAD` before citing SHAs (tip at status write: **`59d44c2`**).
...
| S5-R2-b                  | **LANDED_WITH_DEBT** — `typeAware` on; `typeCheck` off (→ R2-f); node-types gap fixed on 7 pack leaves | [implementation](../reports/ts-project-references-implementation.md) § S5-R2-b · [#161](https://github.com/secundant/neodx/issues/161)                                                 |
...
**Next (S5 residuals):** R2-b **done** (typeAware on, LANDED_WITH_DEBT). R2-f (test tsconfig matrix)
now owns the `typeCheck` flip — unblocks the source-vs-packed-dts conflict in test files. Then R2-d/e
(ATTW + paired `.d.mts`/`.d.cts`, #164). Do not pair R2-a with a product 1.0 Changeset.
